...
public class PrototypePromptTemplate {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            13. 底部 TabBar 数量强制规则（若需求涉及 TabBar）：
             ...
             16. 严禁使用纯 CSS 堆砌/手搓具象实物简笔画（如扭蛋机、汽车、人物、盲盒等）：
                 - 严禁使用多个带 border-radius 的彩色圆球、彩色 div 方块去手搓拼凑扭蛋机、汽车、人物、宠物、盲盒等具象物理实物或伪插画（画风极其幼稚廉价，严重破坏产品原型质感）。
                 - 凡涉及具象主体（扭蛋机、盲盒、手办、角色、商品展示位等），必须在页面中央舞台使用真实素材占位符：<img src="__PROTOTYPE_PRIMARY_ASSET__" class="primary-visual">。
                 - 若没有图片，必须使用精致有质感的卡片展台、半透明毛玻璃面板、奖池网格列表与主色操作区，严禁自行用彩色几何色块伪造涂鸦式简笔画。
             17. 互动/扭蛋机/盲盒/抽奖类页面完整信息架构（严禁生成单薄空洞的页面）：
                 - 严禁只放标题和两个孤立按钮导致下半屏大面积空白！标准移动端页面必须具备完整的版面闭环：
                   ① 顶部栏：返回键、页面标题、用户扭蛋币/金币余额标签、活动规则入口；
                   ② 居中舞台区：核心扭蛋机/盲盒视觉区（预留素材位，可加光效与立体投影）；
                   ③ 核心互动区：单抽按钮、十连抽按钮（标注消耗币数或折扣，高光主色质感）；
                   ④ 延展展示区（自然充实版面）：本期 SSR/SR 盲盒奖品预览卡片横滑/网格展柜、中奖概率公示、最新抽中用户弹幕/跑马灯。
             18. 页面背景全局铺满根节点：
                 - 移动端页面背景色或渐变必须直接写在 html, body 或 100%% 外壳容器上，自顶向下自然延展铺满视口，禁止仅包裹局部小卡片而在下方露出一大片惨白底色。
             """;
