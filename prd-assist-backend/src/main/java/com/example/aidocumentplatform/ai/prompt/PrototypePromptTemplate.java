...
public class PrototypePromptTemplate {
    ...
    private String buildGenerationStrategy(Platform platform, boolean hasReferenceImage) {
        ...
        return """
                ...
                - 色彩：主色 #2457d6，深主色 #1745ba，背景 #f4f7fb，面板 #ffffff，边框 #dfe5ee，强边框 #cdd6e3，正文 #13213a，次级文字 #66738a。
                - 字号：页面标题 22~24px，区块标题 16~18px，正文 14px，辅助文字 12~13px；禁止小于 12px。
                - 尺寸：按钮高度 40px，分段控制 34px，输入框高度 38~40px，表格行高 48px，导航项高度 42px。
                - 间距：使用 8/12/14/20/26/30px 节奏；桌面主内容 padding 26px 30px，移动端 padding 16px。
                - 圆角：控件 8px，卡片/面板 10~11px，品牌标识 9px；不要到处使用超大胶囊圆角。
                - 阴影：面板使用极轻阴影 0 5px 18px rgba(24,43,74,.04)，控件使用 0 2px 5px rgba(20,35,60,.06)。
    ...
    private String buildComponentSpecification() {
        return """
                ...
                1. 全局布局
                   - 桌面页面默认使用 100vw / min-height:100vh，不要生成窄小孤立画布。
                   - 后台/工作台布局：顶部栏 72~76px；左侧栏固定 248px；右侧面板 300~360px；主内容区自适应。
                   - 左侧栏禁止使用 30vw、33%、40% 这类比例宽度；桌面端不得超过 260px，除非用户明确要求超宽目录。
                   - 主内容区 padding 26px 30px 40px；区块之间 gap 14~20px；内容最大宽度根据页面类型控制，不要所有内容挤在一列。
                   - 登录注册页不使用后台侧边栏；默认居中卡片宽 380~440px，或左右分栏 48%/52%。
                   - 内容详情页正文宽 720~960px，行高 1.7，右侧目录宽 220~280px。
                   - 数据看板可使用 4 列指标卡；主工作区可使用 1.55fr / 0.85fr 双栏；窄屏自动单列。

                1.2 内容安全边距（非常重要）
                   - 任何有边框、背景色、圆角或阴影的容器，内部文字必须距离外边框至少 14px。
                   - 卡片/面板/表单分组/步骤块默认 padding 20px；指标卡 padding 18px；小提示块 padding 13px 15px。
                   - 横向步骤卡、流程卡、编号卡（例如“1. 上传规则”“2. 字段识别”）必须 padding 14~16px，标题和说明之间 gap 6~8px。
                   - 横向步骤条每个步骤块必须使用 display:flex 或 grid 布局时保留内部 padding，不允许标题贴着左上角。
                   - 侧边栏品牌区左右 padding 20px；导航列表整体 padding 22px 12px；导航项内部 padding 0 11px 或 11px 12px。
                   - 标题不能贴着容器左上角：容器内第一个标题与容器边框至少 14px，标题下方说明 margin-top 6~8px。
                   - 表单 label 与输入框之间 gap 8px；连续表单项之间 gap 18~22px。
                   - 禁止出现文字与外层 div 边框距离小于 8px 的情况，除非是表格内分割线和极小状态标签。

                1.4 页面类型规范
                   - 登录页：auth-shell 居中或左右分栏；auth-card 宽 420px、padding 32px；auth-visual padding 48px；只保留必要登录内容。
                   - 工作台/后台首页：app-shell + sidebar + app-header + page-main；指标区用 metric-card 四列；内容区 gap 20px。
                   - CRUD 表格页：page-title 区 + toolbar/filter-bar + table-panel + data-table + 分页；表格操作列使用 action-cell/table-actions。
                   - 表单流程页：steps/step-list 在顶部，step-item padding 14px 16px；下方 form-panel 两列表单，右侧 summary-panel。
                   - 内容详情页：detail-panel 阅读宽 720~960px，padding 24px；目录/摘要面板 280~320px。
                   - 数据看板页：metric-card + chart panel + table-panel；图表面板 padding 20px，图表周围留白至少 18px。
                   - 设置页：form-panel 分组布局，每个 setting-card padding 20px，开关/按钮右对齐。
                   - 弹窗/抽屉：modal-card padding 24px，footer 操作区 gap 12px；空/错/成功状态使用 center-state padding 30px。

                1.3 登录/注册页内容规则
                   - 登录页必须保持轻量：品牌/系统名、1 句说明、账号输入、密码输入、记住/忘记密码、主按钮、1 个次要入口即可。
                   - 禁止在登录页左侧或底部堆叠功能模块、规则分析、配置管理、部署说明、长列表、表格或大段介绍。
                   - 注册页只展示必要字段；除非用户明确要求，不要出现复杂权益卡片、营销模块、产品功能介绍区。
                   - 主按钮宽度与表单对齐，高度 40px；表单卡片 padding 28~32px。

                2. 顶部栏 Header
                   - 高度 72~76px，左右 padding 30px，白色/半透明浅色背景，底部 1px 边框。
                   - 左侧放系统名/页面标题，中间可放搜索，右侧放 1~3 个主要操作/用户入口。
                   - 标题 20px，副标题/面包屑 10~12px，操作按钮高度与全局按钮一致。

                3. 侧边栏 Sidebar
                   - 宽度固定 248px，深色企业侧栏背景 #132947，右侧 1px 边框 #203654；若用户明确要求浅色侧栏，使用 #ffffff / #f8fafc。
                   - 侧边栏必须写固定宽度：width/flex-basis 240px 左右，并设置 flex-shrink:0；不要让它占据半屏。
                   - 品牌区高度 76px，左右 padding 20px；品牌标识 36px，圆角 9px。
                   - 侧边栏内所有文字不得贴边：品牌、分组标题、导航项、底部操作都必须在 navList/panel 内有 12~20px 外围留白。
                   - 导航项高度 42px，左右 padding 11~12px，圆角 8px，间距 4~6px。
                   - 一级菜单 13~14px / 500，分组标题 10px uppercase/letter-spacing；选中项使用半透明浅色背景，不要大面积高饱和色块。
                   - 图标可选，尺寸 16~18px；没有明确必要时不要给每个菜单硬加图标。

                4. 主内容区 Main
                   - 页面标题区包含标题、说明、主要操作；标题 22~24px，说明 13~14px。
                   - 筛选/工具栏放在内容上方，使用横向换行布局，gap 8~12px。
                   - 卡片/面板使用白底、1px 边框、10~11px 圆角、轻阴影；面板 padding 20px，指标卡 padding 18px；禁止卡片套卡片。
                   - 表单、表格、列表按真实业务密度排布，不要用大量装饰块替代数据。

                5. 字号与文字
                   - 页面 H1 22~24px，H2 18~20px，H3/卡片标题 16px，正文 14px，辅助文字 12~13px。
                   - 正文颜色 #1f2937，辅助文字 #6b7280，占位/弱提示 #9ca3af。
                   - 行高：标题 1.25~1.35，正文 1.55~1.7，表格 1.4。
                   - 禁止 viewport 线性缩放字号；禁止负 letter-spacing；禁止为了塞内容把字调到 10px。

                6. 按钮
                   - 默认高度 40px，小按钮 34px，大按钮 40px；左右 padding 12~15px，圆角 8px，字重 600。
                   - 主按钮：主色背景 + 白字；次按钮：白底 + 边框；文本按钮：无背景，仅主色文字。
                   - 每个按钮必须有 hover、active、focus 状态；active 可轻微 scale(0.98)，focus 使用 2px ring。
                   - 同一工具栏按钮高度必须一致；不要生成过大的胶囊按钮和渐变按钮，除非用户明确要求。
                   - 同一个表格“操作”列内的按钮必须同高同宽：高度 40px，最小宽度 88px，padding 0 15px；不要因为文字长短不同导致按钮大小不同。

                7. 输入框/表单
                   - 输入框、选择器高度 36~40px，padding 8~12px，圆角 8px，边框 #d1d5db。
                   - label 13~14px，错误提示 12px，表单项垂直间距 14~18px。
                   - 复杂表单使用两列或分组；移动端自动单列。

                8. 表格/列表
                   - 表头 13px / 600，正文 14px，行高 48px，单元格 padding 12px 20px。
                   - 行号/序号/ID 列字号不得小于 13px，列宽 56~72px，不要缩到难以阅读。
                   - 表格顶部保留筛选区和操作栏，底部保留分页/状态。
                   - 状态标签高度 22~24px，字号 12px，颜色克制；不要每行堆多个彩色徽章。
                   - 表格操作列按钮统一尺寸和对齐；若有多个操作按钮，使用 8px gap，禁止一个宽一个窄。

                8.1 用户规范/参考图覆盖规则
                   - 用户输入其他设计规范时，只覆盖颜色、质感、圆角、阴影、按钮外观、卡片风格、字体气质等视觉 token。
                   - 上传参考图时，同样只覆盖视觉 token；默认不覆盖侧栏宽度、表格行高、按钮高度、主内容 padding、登录页内容结构。
                   - 只有用户明确写“按参考图布局/侧边栏要更宽/按钮要更大/登录页要展示功能介绍”时，才允许改变布局尺寸和内容复杂度。

                9. 禁止项
                   - 禁止 emoji、图标墙、无意义 SVG 插画、满屏渐变、超大英雄区、营销落地页式排版。
                   - 禁止元素重叠、文字溢出、按钮文字挤压、横向滚动失控。
                   - 除非用户明确要求，否则不要改变上述布局尺寸和组件规格。
                """;
    }
    ...
    private String getStyleGuide(Platform platform) {
        return switch (platform) {
            case APP -> """
                    【终端约束】App 端设计，按手机 App 原型生成，默认画布 390px 居中。
    ...
    private String buildPlatformDesignSpec(Platform platform) {
        return switch (platform) {
            case APP -> """
                    ...
                    1. 画布与安全区
                       - 使用 .mobile-shell 或 .phone-shell，宽度 390px，max-width:100%，min-height:100vh，居中展示。
                       - body 背景可以是 #eef4f7，手机画布背景 #f7f9fb；内容区 padding 16px。
                    ...
                    2. 导航结构
                       ...
                       - 页面主内容 .mobile-content 必须为单列纵向内容流，底部留出 72px 以上空间避免被 TabBar 遮挡。
                    3. 组件尺寸
                       - 主要按钮高度 44~48px，圆角 10~12px，整行按钮可 width:100%；每屏最多 1 个强主按钮。
                       - 输入框/选择器/搜索框高度 44~48px，字号 15~16px，label 14px，说明/错误 12~13px。
                       - 列表项高度 52~64px，左右 padding 16px；卡片 padding 14~16px，圆角 12~14px，卡片间距 12px。
                    ...
                    4. 页面类型
                       ...
                       - 详情：标题区 + 信息分组 + 底部固定主操作；正文行高 1.6。
                       - 表单流程：步骤用纵向 step 或顶部轻量步骤；表单单列；提交按钮固定底部或跟随表单底部。
                       - 个人中心/设置：头像/账号区 + cell 列表 + 分组标题，遵循 WeUI/Vant cell 模式。
                    5. 视觉风格
                       - 默认采用 OPPO / ColorOS 风格方向：轻盈、水生感、清透背景、柔和层次、圆润但不夸张、信息分组清楚。
                       - 主色使用 OPPO/ColorOS 倾向的清爽蓝绿色：#0bb6c7 或 #1677ff，页面背景 #f7f9fb，卡片 #ffffff，正文 #111827，辅助文字 #6b7280。
                       - 卡片使用低饱和浅色背景或白底细边框，阴影极轻：0 8px 24px rgba(15, 35, 55, .06)。
                       - 按钮使用圆润矩形，不使用大面积强渐变；主按钮可以使用 #0bb6c7，次按钮使用浅色填充或细边框。
            ...
            case MINI_PROGRAM -> """
                    ...
                    1. 画布宽度 390px，内容区 padding 16px，页面背景 #f7f8fa。
                    2. 顶部使用小程序标题栏/导航栏，高度 44~56px；常用返回、首页、标题，触控区不小于 44px。
                    3. 底部可用 TabBar 或固定操作栏，高度 56~64px，并使用 safe-area inset。
                    4. 信息承载优先使用 cell、card、form、popup、action-sheet、toast、empty、result；禁止 Web 后台侧边栏和桌面表格。
                    5. 列表项 52~64px，按钮 44~48px，输入框 44~48px，卡片 padding 14~16px，间距 12~16px。
                    """;
            case PAD -> """
