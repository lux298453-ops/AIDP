-- ============================================================
-- 策划部 AI 辅助设计平台 — 数据库建表脚本（参考文档）
-- 数据库: prd_assist / MySQL 8.0+
-- 说明: 开发阶段由 JPA ddl-auto=update 自动建表，
--       此文件作为 SQL 参考，生产环境可用 Flyway 管理版本
-- ============================================================

CREATE DATABASE IF NOT EXISTS `prd_assist`
...
CREATE TABLE `async_task` (
    ...
    `result_ref_id` BIGINT UNSIGNED NULL COMMENT '成功后指向对应结果表的ID',
...
CREATE TABLE `prd_document` (
    `id`             BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `user_id`        BIGINT UNSIGNED NOT NULL,
    `task_id`        BIGINT UNSIGNED NULL COMMENT '关联生成该文档的任务',
    ...
    `source_type`    VARCHAR(20)  NOT NULL DEFAULT 'MANUAL' COMMENT 'MANUAL/XMIND',
...
CREATE TABLE `prototype_result` (
    `id`                 BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `user_id`            BIGINT UNSIGNED NOT NULL,
    `prd_document_id`    BIGINT UNSIGNED NULL COMMENT '关联的PRD',
    ...
    `content`            JSON         NOT NULL COMMENT '生成的页面代码/结构',
...
CREATE TABLE `review_report` (
    ...
    `issues`          JSON NOT NULL COMMENT '问题列表',
    `created_at`      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
