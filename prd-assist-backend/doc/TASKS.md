...
- [x] PRD 审查结果点击定位章节不稳定：审查报告 issue 已增加 `chapterIndex` / `chapterTitle` 结构化定位字段，前端优先按章节锚点跳转，旧报告继续保留模糊匹配兜底。
- [x] PRD 生成内容中的 Markdown 表格未渲染为真实表格：前端编辑/预览已将 `|...|` 表格块渲染为 `<table>`，Word 导出已转换为 `XWPFTable`。
