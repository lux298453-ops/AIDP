...
export async function GET(request: Request) {
  ...
  const cacheKey = `content:${range.data}`;
  ...
  const { data, error } = await client.rpc("admin_analytics_content", {
  ...
  const parsed = analyticsContentSchema.safeParse(data);
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Content analytics returned an invalid payload.", code: "ANALYTICS_INVALID_PAYLOAD" },
